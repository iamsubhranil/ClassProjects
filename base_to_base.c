...
// ==================================================
// Bookkeeping methods, to reduce the usage of headers
// ===================================================
...
int str_len(char *s){
    int len = 0; // Reset the counter
    while(s[len] != '\0') // Loop until the string terminates
        len++;
    return len;
}

// Returns the base^exponent of a number
long int pwr(long int base, long int exp){
    long int bak = base;
    if(exp < 1)
        return 1;
    while(exp > 1){
